...
My_path = 'D:\\coding_file\\study\\Lesson\\oo_lesson\\OO\\test\\hw2'
...
class get_Code:
    ...
    def get_Factor(self):
        ...
        Factor_list = [self.get_Con(), self.get_Exp(),
                       self.get_Var()]
    ...
    def get_Tri(self):
        ...
        return Tri[random3]+"("+self.get_Factor()+")"

    def main(self):
        R = ['+', '-', '*']
        Main = [self.get_Expr(), self.get_Tri()]
        self.code = self.get_Poly()
        i = 0
        for i in range(0, 5):
            ran = random.randint(0, len(R)-1)
            self.code = self.code+str(R[ran]) + \
                Main[random.randint(0, len(Main)-1)]
            i += 1
