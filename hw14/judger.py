...
# set names here
playerName = ['Charles','dt','gx','sj']
# playerName = ['Charles','gx']
# playerName = ['dt']
work_dir='D:\\coding_file\\study\\Lesson\\oo_lesson\\OO\\test\\hw14'
times = []
ans = []
lineNum = []
...
def runAndGetAns(recodeTime):
    for i in range(playerNum):
        ...
        with open(name+'_stdout.txt', 'r') as f:
            with open(name+'_ans_stdout.txt', 'w') as r:
              lists=f.readlines()
              res=[]
              for j in range(len(lists)):
                line = lists[j]
                if str(line).startswith('(State)')==True:
                    continue
